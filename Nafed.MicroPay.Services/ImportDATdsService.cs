...
{
    public class ImportDATdsService : BaseService, IImportDAtdsService
    {
        private readonly IExcelService excelService;
        private readonly IDATdsRepository daTdsRepo;
        public ImportDATdsService(IExcelService excelService, IDATdsRepository daTdsRepo)
        {
            this.daTdsRepo = daTdsRepo;
            this.excelService = excelService;
        }

        public IEnumerable<dynamic> ReadImportMonthlyExcelData(string fileFullPath, bool isHeader, out string message, out int error, out int warning, out List<Model.ImportDATDSData> DATA, out List<string> missingHeaders, out List<string> columnName)
        {
            ...
            try
            {
                ...
                DATA = new List<Model.ImportDATDSData>();
                ...
                string[] header =
                    {
                      "Employee Code",
                     "Employee Name","Branch Name","Designation Name", "Month","Year", "DA","TDS","NetPay"
                };
                ...
                if (isValidHeaderList)
                {
                    ...
                    if (dataWithHeader.Rows.Count > 0)
                    {
                        ...
                        Model.DATDSColValues colWithDistinctValues = GetDistinctValuesOfImportData(dataWithHeader);
                        ...
                        try
                        {
                            ...
                            dtEmpPfNoFromDB = daTdsRepo.GetImportFieldValues(dtEmpCodes, dtBranch);
                            MergeMonthlySheetDataWithDBValues(dataWithHeader, dtEmpPfNoFromDB.Tables[0], dtEmpPfNoFromDB.Tables[1]);
                        }
                        catch (Exception ex)
                        {
                        ...
                        ValidateDATDSImportSheetData(dataWithHeader, out error, out warning);
                        ...
                        {
                            Model.ImportDATDSData MI = new Model.ImportDATDSData();
                            for (int j = 0; j < dataWithHeader.Columns.Count; j++)
                            {
                                ...
                                if (sColName == "BranchName")
                                ...
                                else if (sColName == "DesignationName")
                                    ...
                                    MI.DA = dataWithHeader.Rows[i][sColName].ToString().Trim();
        ...
        private Model.DATDSColValues GetDistinctValuesOfImportData(DataTable dtImportData)
        {
            log.Info($"ImportPfBalanceService/GetDistinctValuesOfImportData");
            try
            {
                Model.DATDSColValues colWithDistinctValues = new Model.DATDSColValues();
        ...
        void ValidateDATDSImportSheetData(DataTable dtSource, out int iError, out int iWarning)
        {
            iError = 0; iWarning = 0;
        ...
        public dynamic RemoveProperty(Model.ImportDATDSData model)
        {
            try
            {
        ...
        public Model.ImportDATDSData AddCellValue(Model.ImportDATDSData input, string keyName, string cellValue)
        {
            try
            {
